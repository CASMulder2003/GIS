...
# Check to see that teh correct borough has been pulled out
tm_shape(Harrow) + 
...
# grid overlaid across teh windowBluePlaquesSub.ppp2 <- BluePlaquesSub.ppp %>%
BluePlaquesSub.ppp %>%
...
teststats2 <- quadrat.test(BluePlaquesSub.ppp, nx = 6, ny = 6)
...
# First checking teh coordinate reference system of the Harrow spatial polygon: 
...
```{r}
# First extracting the poitns from teh spatial points data frame
...
1.  `fill` is for the interior colour (e.g. polygons)

2.  `color` is for borders and points
