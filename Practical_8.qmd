---
title: "Practical 6 - Explaining Spatial Patterns"
format: html
editor: visual
---

## Practical 6 – Explaining Spatial Patterns
...
```{r}
# List .zip files in prac7_data
zip_files <- dir_info(here::here("prac7_data")) %>%
...
# Unzip each file
lapply(as.character(zip_files), unzip, exdir = here::here("prac7_data"))
```
...
Londonwards <- fs::dir_info(here::here("prac7_data", 
                                       ...
                                       "ESRI")) %>%
  # $ means exact match at end of string
  dplyr::filter(str_detect(path, 
                           regex("London_Ward_CityMerged\\.shp$", ignore_case = TRUE))) %>%
  dplyr::select(path) %>%
  dplyr::pull() %>%
  # read in the file
  sf::st_read()
...
$β_0$ is the intercept (the value of $y$ when $x=0$ - somewhere around 370 on the graph above);

$β_1$ is sometimes referred to as the ‘slope’ parameter and is simply the change in the value of $y$ for a 1 unit change in the value of $x$ (the slope of the blue line) - reading the graph above, the change in the value of $y$ reading between 1.0 and 2.0 on the $x$ axis looks to be around -40.

$ϵ_i$ is a random error term (positive or negative) that should sum to 0 - esentially, if you add all of the vertical differences between the blue line and all of the residuals, it should sum to 0.
...
**Coefficient Estimates** - these are the $β_0$ (intercept) and $β_1$ (slope) parameter estimates from Equation 1. You will notice that at $β_0=371.471$ and $β_1=−41.237$ they are pretty close to the estimates of 370 and -40 that we read from the graph earlier, but more precise.
...
> Should I enter and scale my data
...
```         
## # A tibble: 3 × 5 ##   term                                     estimate std.error statistic  p.value ##   <chr>                                       <dbl>     <dbl>     <dbl>    <dbl> ## 1 (Intercept)                                 202.      20.1      10.0  4.79e-22 ## 2 unauthorised_absence_in_all_schools_per…    -36.2      1.92    -18.9  3.71e-63 ## 3 log(median_house_price_2014)                 12.8      1.50      8.50 1.37e-16
```

```         
## # A tibble: 1 × 12 ##   r.squared adj.r.squared sigma statistic  p.value    df logLik   AIC   BIC ##       <dbl>         <dbl> <dbl>     <dbl>    <dbl> <dbl>  <dbl> <dbl> <dbl> ## 1     0.483         0.482  15.5      291. 4.78e-90     2 -2604. 5215. 5233. ## # ℹ 3 more variables: deviance <dbl>, df.residual <int>, nobs <int>
```
...
Figure 8.1: Word anatomy of multicollinearity. Source: [What the Heck is Multicollinearity?, Andrew Ozbun](https://medium.com/geekculture/what-the-heck-is-multicollinearity-c5582ddbb2f7)
...
```         
## unauthorised_absence_in_all_schools_percent_2013  ##                                         1.105044  ##                     log(median_house_price_2014)  ##                                         1.105044
```
...
```         
## # A tibble: 1 × 5 ##   statistic p.value autocorrelation method             alternative ##       <dbl>   <dbl>           <dbl> <chr>              <chr>       ## 1      1.61       0           0.193 Durbin-Watson Test two.sided
```
...
[+](https://andrewmaclachlan.github.io/CASA0005repo/explaining-spatial-patterns.html# "Zoom in")[−](https://andrewmaclachlan.github.io/CASA0005repo/explaining-spatial-patterns.html# "Zoom out")

TYPE

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%226.2%22%20height%3D%226.2%22%3E%0A%20%20%3Ccircle%20id%3D%22circle%22%20cx%3D%223.1%22%20cy%3D%223.1%22%20r%3D%222.1%22%20stroke%3D%22transparent%22%20fill%3D%22%23000000%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Fcircle%3E%0A%3C%2Fsvg%3E) Academy Converter

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%226.2%22%20height%3D%226.2%22%3E%0A%20%20%3Ccircle%20id%3D%22circle%22%20cx%3D%223.1%22%20cy%3D%223.1%22%20r%3D%222.1%22%20stroke%3D%22transparent%22%20fill%3D%22%23000000%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Fcircle%3E%0A%3C%2Fsvg%3E) Academy Sponsor Led

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%226.2%22%20height%3D%226.2%22%3E%0A%20%20%3Ccircle%20id%3D%22circle%22%20cx%3D%223.1%22%20cy%3D%223.1%22%20r%3D%222.1%22%20stroke%3D%22transparent%22%20fill%3D%22%23000000%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Fcircle%3E%0A%3C%2Fsvg%3E) Community School

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%226.2%22%20height%3D%226.2%22%3E%0A%20%20%3Ccircle%20id%3D%22circle%22%20cx%3D%223.1%22%20cy%3D%223.1%22%20r%3D%222.1%22%20stroke%3D%22transparent%22%20fill%3D%22%23000000%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Fcircle%3E%0A%3C%2Fsvg%3E) Foundation School

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%226.2%22%20height%3D%226.2%22%3E%0A%20%20%3Ccircle%20id%3D%22circle%22%20cx%3D%223.1%22%20cy%3D%223.1%22%20r%3D%222.1%22%20stroke%3D%22transparent%22%20fill%3D%22%23000000%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Fcircle%3E%0A%3C%2Fsvg%3E) Free Schools

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%226.2%22%20height%3D%226.2%22%3E%0A%20%20%3Ccircle%20id%3D%22circle%22%20cx%3D%223.1%22%20cy%3D%223.1%22%20r%3D%222.1%22%20stroke%3D%22transparent%22%20fill%3D%22%23000000%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Fcircle%3E%0A%3C%2Fsvg%3E) Studio Schools

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%226.2%22%20height%3D%226.2%22%3E%0A%20%20%3Ccircle%20id%3D%22circle%22%20cx%3D%223.1%22%20cy%3D%223.1%22%20r%3D%222.1%22%20stroke%3D%22transparent%22%20fill%3D%22%23000000%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Fcircle%3E%0A%3C%2Fsvg%3E) University Technical College

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%226.2%22%20height%3D%226.2%22%3E%0A%20%20%3Ccircle%20id%3D%22circle%22%20cx%3D%223.1%22%20cy%3D%223.1%22%20r%3D%222.1%22%20stroke%3D%22transparent%22%20fill%3D%22%23000000%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Fcircle%3E%0A%3C%2Fsvg%3E) Voluntary Aided School

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%226.2%22%20height%3D%226.2%22%3E%0A%20%20%3Ccircle%20id%3D%22circle%22%20cx%3D%223.1%22%20cy%3D%223.1%22%20r%3D%222.1%22%20stroke%3D%22transparent%22%20fill%3D%22%23000000%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Fcircle%3E%0A%3C%2Fsvg%3E) Voluntary Controlled School

model2resids

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23D7191C%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -73.26 to -25.49

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23FDAE61%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -25.49 to -9.08

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23FFFFBF%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -9.08 to 2.78

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23ABD9E9%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) 2.78 to 17.51

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%232C7BB6%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) 17.51 to 50.82
...
```         
## # A tibble: 1 × 7 ##   estimate1 estimate2 estimate3 statistic  p.value method            alternative ##       <dbl>     <dbl>     <dbl>     <dbl>    <dbl> <chr>             <chr>       ## 1     0.282   -0.0016  0.000556      12.0 1.54e-33 Moran I test und… greater
```

```         
## # A tibble: 1 × 7 ##   estimate1 estimate2 estimate3 statistic  p.value method            alternative ##       <dbl>     <dbl>     <dbl>     <dbl>    <dbl> <chr>             <chr>       ## 1     0.292   -0.0016  0.000718      10.9 3.78e-28 Moran I test und… greater
```
...
```         
## # A tibble: 4 × 5 ##   term                                     estimate std.error statistic  p.value ##   <chr>                                       <dbl>     <dbl>     <dbl>    <dbl> ## 1 rho                                       5.16e-3   0.00759     0.679 4.97e- 1 ## 2 (Intercept)                               2.02e+2  20.1        10.1   0        ## 3 unauthorised_absence_in_all_schools_per… -3.62e+1   1.91      -18.9   0        ## 4 log(median_house_price_2014)              1.26e+1   1.53        8.21  2.22e-16
```

```         
## # A tibble: 1 × 6 ##   r.squared   AIC   BIC deviance logLik  nobs ##       <dbl> <dbl> <dbl>    <dbl>  <dbl> <int> ## 1     0.484 5217. 5239.  150150. -2604.   626
```
...
```         
## Impact measures (lag, exact): ##                                                    Direct    Indirect     Total ## unauthorised_absence_in_all_schools_percent_2013 -36.1758 -0.18732201 -36.36312 ## log(median_house_price_2014)                      12.5879  0.06518144  12.65308
```
...
```         
## Impact measures (lag, trace): ##                                                      Direct   Indirect ## unauthorised_absence_in_all_schools_percent_2013 -29.581804 -18.450157 ## log(median_house_price_2014)                       9.908963   6.180215 ##                                                      Total ## unauthorised_absence_in_all_schools_percent_2013 -48.03196 ## log(median_house_price_2014)                      16.08918
```

```         
## Impact measures (lag, trace): ##                                                      Direct   Indirect ## unauthorised_absence_in_all_schools_percent_2013 -29.581911 -18.450050 ## log(median_house_price_2014)                       9.908999   6.180179 ##                                                      Total ## unauthorised_absence_in_all_schools_percent_2013 -48.03196 ## log(median_house_price_2014)                      16.08918
```

We can also get the p-values (where an R is set, this is the number of simulations to use)…from the sparse computation

```         
## Impact measures (lag, trace): ##                                                      Direct   Indirect ## unauthorised_absence_in_all_schools_percent_2013 -29.581804 -18.450157 ## log(median_house_price_2014)                       9.908963   6.180215 ##                                                      Total ## unauthorised_absence_in_all_schools_percent_2013 -48.03196 ## log(median_house_price_2014)                      16.08918 ## ======================================================== ## Simulation results ( variance matrix): ## ======================================================== ## Simulated standard errors ##                                                    Direct Indirect    Total ## unauthorised_absence_in_all_schools_percent_2013 1.764654 2.857632 3.571394 ## log(median_house_price_2014)                     1.608647 1.149078 2.473060 ##  ## Simulated z-values: ##                                                      Direct  Indirect ## unauthorised_absence_in_all_schools_percent_2013 -16.759294 -6.420621 ## log(median_house_price_2014)                       6.139398  5.297483 ##                                                       Total ## unauthorised_absence_in_all_schools_percent_2013 -13.418323 ## log(median_house_price_2014)                       6.454896 ##  ## Simulated p-values: ##                                                  Direct     Indirect   ## unauthorised_absence_in_all_schools_percent_2013 < 2.22e-16 1.3572e-10 ## log(median_house_price_2014)                     8.2835e-10 1.1741e-07 ##                                                  Total      ## unauthorised_absence_in_all_schools_percent_2013 < 2.22e-16 ## log(median_house_price_2014)                     1.0829e-10
```
...
```         
## # A tibble: 4 × 5 ##   term                                     estimate std.error statistic  p.value ##   <chr>                                       <dbl>     <dbl>     <dbl>    <dbl> ## 1 rho                                         0.374    0.0409      9.14 0        ## 2 (Intercept)                               116.      20.1         5.76 8.39e- 9 ## 3 unauthorised_absence_in_all_schools_per…  -28.5      1.97      -14.5  0        ## 4 log(median_house_price_2014)                9.29     1.48        6.28 3.36e-10
```
...
```         
## # A tibble: 1 × 7 ##   estimate1 estimate2 estimate3 statistic p.value method             alternative ##       <dbl>     <dbl>     <dbl>     <dbl>   <dbl> <chr>              <chr>       ## 1    0.0468   -0.0016  0.000717      1.81  0.0353 Moran I test unde… greater
```
...
-   ξi is the spatial component of the error terms…in other words the residuals of the values in surrounding spatial units based on the weight matrix.

-   λ is a measure of correlation between neighbouring residuals.. “it indicates the extent to which the spatial component of the errors ξi are correlated with one another for nearby observations” as per the weight matrix, Ward and Gleditsch (2008). **If there is no correlation between the then this defaults to normal OLS regression**
...
```         
## # A tibble: 4 × 5 ##   term                                     estimate std.error statistic  p.value ##   <chr>                                       <dbl>     <dbl>     <dbl>    <dbl> ## 1 (Intercept)                               154.      28.2         5.44 5.28e- 8 ## 2 unauthorised_absence_in_all_schools_per…  -32.3      2.22      -14.5  0        ## 3 log(median_house_price_2014)               16.2      2.12        7.62 2.55e-14 ## 4 lambda                                      0.475    0.0451     10.5  0
```
...
```         
##  [1] "ward_code"                "pct_shared_ownership2011" ##  [3] "pct_rent_free2011"        "candidate"                ##  [5] "inner_outer"              "x"                        ##  [7] "y"                        "avg_gcse2011"             ##  [9] "unauth_absence_schools11" "geometry"
```
...
```         
## # A tibble: 85 × 2 ##    All_variables   Variable_class ##    <chr>           <chr>          ##  1 name            character      ##  2 gss_code        character      ##  3 hectares        numeric        ##  4 nonld_area      numeric        ##  5 lb_gss_cd       character      ##  6 borough         character      ##  7 poly_id         numeric        ##  8 ward_name       character      ##  9 old_code        character      ## 10 population_2015 numeric        ## # ℹ 75 more rows
```

```         
## # A tibble: 4 × 5 ##   term                                     estimate std.error statistic  p.value ##   <chr>                                       <dbl>     <dbl>     <dbl>    <dbl> ## 1 (Intercept)                                  97.6     24.1       4.06 5.62e- 5 ## 2 unauthorised_absence_in_all_schools_per…    -30.1      2.03    -14.8  7.02e-43 ## 3 log(median_house_price_2014)                 19.8      1.74     11.4  2.09e-27 ## 4 inner_outerOuter                             10.9      1.51      7.24 1.30e-12
```
...
```         
## # A tibble: 4 × 5 ##   term                                     estimate std.error statistic  p.value ##   <chr>                                       <dbl>     <dbl>     <dbl>    <dbl> ## 1 (Intercept)                                 109.      23.2       4.68 3.53e- 6 ## 2 unauthorised_absence_in_all_schools_per…    -30.1      2.03    -14.8  7.02e-43 ## 3 log(median_house_price_2014)                 19.8      1.74     11.4  2.09e-27 ## 4 inner_outerInner                            -10.9      1.51     -7.24 1.30e-12
```
...
#### **8.6.6.1** A few things to watch out for…
...
## **8.7** Task 3 - Spatial Non-stationarity and Geographically Weighted Regression Models (GWR)

Here’s my final model from the last section:

```         
## # A tibble: 6 × 5 ##   term                                     estimate std.error statistic  p.value ##   <chr>                                       <dbl>     <dbl>     <dbl>    <dbl> ## 1 (Intercept)                               240.      27.9         8.59 7.02e-17 ## 2 unauthorised_absence_in_all_schools_per…  -23.6      2.16      -10.9  1.53e-25 ## 3 log(median_house_price_2014)                8.41     2.26        3.72 2.18e- 4 ## 4 inner_outerInner                          -10.4      1.65       -6.30 5.71e-10 ## 5 rate_of_job_seekers_allowance_jsa_claim…   -2.81     0.635      -4.43 1.12e- 5 ## 6 percent_with_level_4_qualifications_and…    0.413    0.0784      5.27 1.91e- 7
```
...
![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23713B79%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -80 to -60

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23B26EBA%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -60 to -40

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23D9B0DE%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -40 to -20

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23EBDAED%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -20 to 0

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23D2E4D2%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) 0 to 20

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%239BC99C%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) 20 to 40

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%2348984B%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) 40 to 60

[Leaflet](https://leafletjs.com/ "A JS library for interactive maps") \| Tiles © Esri — Esri, DeLorme, NAVTEQ

lon: -0.23621 \| lat: 51.39261 \| zoom: 9

```         
## # A tibble: 1 × 7 ##   estimate1 estimate2 estimate3 statistic  p.value method            alternative ##       <dbl>     <dbl>     <dbl>     <dbl>    <dbl> <chr>             <chr>       ## 1     0.224   -0.0016  0.000718      8.42 1.91e-17 Moran I test und… greater
```
...
![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%2367001F%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%220.5%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -50 to -40

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23B2182B%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%220.5%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -40 to -30

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23D6604D%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%220.5%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -30 to -20

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23F4A582%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%220.5%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -20 to -10

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23FDDBC7%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%220.5%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -10 to 0

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23D1E5F0%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%220.5%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) 0 to 10
...
![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23D73027%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -40 to -30

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23F46D43%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -30 to -20

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23FDAE61%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -20 to -10

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23FEE090%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) -10 to 0

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23E0F3F8%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) 0 to 10

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%23ABD9E9%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) 10 to 20

![](data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20version%3D%221.1%22%20width%3D%2216%22%20height%3D%2216%22%3E%0A%20%20%3Crect%20id%3D%22stadium%22%20x%3D%221%22%20y%3D%221%22%20height%3D%2214%22%20width%3D%2214%22%20rx%3D%2225%25%22%20stroke%3D%22%23404040%22%20fill%3D%22%2374ADD1%22%20stroke-opacity%3D%221%22%20fill-opacity%3D%221%22%20stroke-width%3D%221%22%20stroke-dash%3D%22%22%20baseSize%3D%2220%22%3E%3C%2Frect%3E%0A%3C%2Fsvg%3E) 20 to 30
